...
//  SAG with Weight Decay regularization
...
// SAG with Average weight decay regularization

template<typename Gtype, typename Wtype, typename Htype>
__global__ void SAGAwdUpdateAllAndClear(int N,
  ...
  CUDA_KERNEL_LOOP(i, N) {
    ...
    float hf = float(h[i]);
    float tf = float(t[i]);
    hf = momentum * hf + m1 * (gf + decay * (wf - tf));
    wf -= rate * hf;
    tf = momentum * tf + m1 * wf;
    w[i] = Wtype(wf);
    h[i] = Htype(hf);
    t[i] = Htype(tf);
    if (clear_grads) {
...
template<>
__global__ void SAGAwdUpdateAllAndClear<half, half, half>(int N,
    half* g, half* w, half* h, half* t,
    float momentum, float rate,  float decay, bool clear_grads) {
  float m1 = 1.F - momentum;
  half hz;
  CUDA_KERNEL_LOOP(i, N) {
    ...
    float gf = __half2float(g[i]);
    float hf = __half2float(h[i]);
    float tf = __half2float(t[i]);
    hf = momentum * hf + m1 * (gf + decay * (wf - tf));
    wf -= rate * hf;
    tf = momentum * tf + m1 * wf;
    w[i] = float2half_clip(wf);
    h[i] = float2half_clip(hf);
    t[i] = float2half_clip(tf);
    if (clear_grads) {
      g[i] = hz;
    }
  }
...
template<>
__global__ void SAGAwdUpdateAllAndClear<float, float,  half>(int N,
    float* g, float* w, half* h, half* t,
    float momentum, float rate,  float decay, bool clear_grads) {
  float m1 = 1.F - momentum;
  CUDA_KERNEL_LOOP(i, N) {
    float wf = w[i];
    float hf = __half2float(h[i]);
    float tf = __half2float(t[i]);
    hf = momentum * hf + m1 * (g[i] + decay * (wf - tf));
    wf -= rate * hf;
    tf = momentum * tf + m1 * wf;
    w[i] = wf;
    h[i] = float2half_clip(hf);
...
template<>
__global__ void SAGAwdUpdateAllAndClear<half, float, float>(int N,
    half* g, float* w, float* h, float* t,
    float momentum, float rate,  float decay, bool clear_grads) {
  float m1 = 1.F - momentum;
  half hz;
  CUDA_KERNEL_LOOP(i, N) {
    ...
    float gf = __half2float(g[i]);
    float tf = t[i];
    float hf = momentum * h[i] + m1 * (gf + decay * (wf - tf));
    wf -= rate * hf;
    tf = momentum * tf + m1 * wf;
    w[i] = wf;
    h[i] = hf;
...
void SAG_reg_update_and_clear_gpu(int N,
  ...
  if (reg_type == "L2") {
    ...
    SAGWdUpdateAllAndClear  // NOLINT_NEXT_LINE(whitespace/operators)
         <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS, 0, stream>>>(N,
           g, w, h, momentum, rate, decay, clear_grads);
  } else if (reg_type == "AWD") {
    SAGAwdUpdateAllAndClear  // NOLINT_NEXT_LINE(whitespace/operators)
         <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS, 0, stream>>>(N,
           g, w, h, t, momentum, rate, decay, clear_grads);
  } else {
...
void SAG_reg_update_and_clear_gpu<float16, float16, float16>(int N,
    float16* g, float16 *w,  float16* h, float16* t,
    float momentum, float rate, const std::string& reg_type, float decay,
    void *handle, bool clear_grads) {
  cublasHandle_t cublas_handle =
      handle == nullptr ? Caffe::cublas_handle(0) : reinterpret_cast<cublasHandle_t>(handle);
  ...
  if (reg_type == "L2") {
    ...
    SAGAwdUpdateAllAndClear  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS, 0, stream>>>(N,
          reinterpret_cast<half*>(g), reinterpret_cast<half*>(w), reinterpret_cast<half*>(h), reinterpret_cast<half*>(t),
          momentum, rate, decay, clear_grads);
  } else {
    LOG(FATAL) << "Unknown regularization mode: " << reg_type;
  }
...
void SAG_reg_update_and_clear_gpu<float, float, float16>(int N,
    float* g, float *w,  float16* h, float16* t,
    float momentum, float rate, const std::string& reg_type, float decay,
    void *handle, bool clear_grads) {
  cublasHandle_t cublas_handle =
      handle == nullptr ? Caffe::cublas_handle(0) : reinterpret_cast<cublasHandle_t>(handle);
  ...
  if (reg_type == "L2") {
    ...
    SAGAwdUpdateAllAndClear  // NOLINT_NEXT_LINE(whitespace/operators)
      <<<CAFFE_GET_BLOCKS(N), CAFFE_CUDA_NUM_THREADS, 0, stream>>>(N,
          g, w, reinterpret_cast<half*>(h), reinterpret_cast<half*>(t),
          momentum, rate, decay, clear_grads);
  } else {
    LOG(FATAL) << "Unknown regularization mode: " << reg_type;
  }
...
void SAG_reg_update_and_clear_gpu<float16, float, float>(int N,
    float16* g, float *w,  float* h, float* t,
    float momentum, float rate, const std::string& reg_type, float decay,
    void *handle, bool clear_grads) {
  cublasHandle_t cublas_handle =
      handle == nullptr ? Caffe::cublas_handle(0) : reinterpret_cast<cublasHandle_t>(handle);
